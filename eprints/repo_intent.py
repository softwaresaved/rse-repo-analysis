...
def main(filepath, true_list_path, false_list_path):
    ...
    df = df[df.page_no < 2]
    created_map = {"repo_id": [], "mention_created": []}
    for path, v in [(true_list_path, "true"), (false_list_path, "false")]:
        with open(path, "r") as f:
            for line in f.readlines():
                created_map["repo_id"].append(line.rstrip("\n"))
                created_map["mention_created"].append(v)
    created_df = pd.DataFrame(created_map)
    merged_df = created_df.merge(df, how="left", left_on="repo_id", right_on="github_user_cleaned_url")
    ...
    corrected_cnt = 0
    idx = merged_df[(merged_df.repo_id == "IraKorshunova/folk-rnn") & (merged_df.pdf_url == "https://eprints.kingston.ac.uk/id/eprint/44298/1/Ben-Tal-O-44298-VoR.pdf")].index[0]
    merged_df.loc[idx, "mention_created"] = False
    corrected_cnt += 1
    print(f"[INFO] Manual corrections: {corrected_cnt}")
    merged_df.to_csv("../data/eprints_w_intent.csv")
